...
import mobi.checkapp.epoc.chat.exceptions.YaximXMPPException;
import mobi.checkapp.epoc.chat.util.ConnectionState;
import mobi.checkapp.epoc.chat.util.LogConstants;
...
public class SmackableImp implements Smackable {
	...
	static final DiscoverInfo.Identity YAXIM_IDENTITY = new DiscoverInfo.Identity("client",
			VolleyControler.XMPP_IDENTITY_NAME,
			VolleyControler.XMPP_IDENTITY_TYPE);
	...
	private static final String PING_ALARM = "mobi.checkapp.epoc.chat.PING_ALARM";
	private static final String PONG_TIMEOUT_ALARM = "mobi.checkapp.epoc.chat.PONG_TIMEOUT_ALARM";
	...
	private synchronized void initXMPPConnection() {
		...
		try {
			...
			MemorizingTrustManager mtm = VolleyControler.getApp(mService).mMTM;
	...
	protected void sendUserWatching() {
		...
		toggle_google_queue.setType(Type.SET);
